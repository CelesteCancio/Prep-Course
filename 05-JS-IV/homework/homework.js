...
function pasarUsuarioAPremium(usuarios) {
  ...
  for (let i=0; i<usuarios.length; i++){
    ...
    for (key in usuarioActual){
      if (usuarioActual[key] === usuarioActual["esPremium"]){
...
function sumarLikesDeUsuario(usuario) {
  ...
  //usuario = {
  //   posts = [{likes:},{likes:},{likes:}];
  // }
  const arrayDePosts = usuario.posts;
  let sumaLikes = 0;
  for (let i=0; i<arrayDePosts.length; i++){
    let arrayDePostsActual = arrayDePosts[i];    
    sumaLikes += arrayDePostsActual.likes;    
  }
  return sumaLikes;
}
